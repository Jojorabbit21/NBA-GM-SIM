...
import { usePlayerAllStarAwards } from '../../../hooks/usePlayerAllStarAwards';
...
const MultiPlayerDetailView: React.FC = () => {
    ...
    // 시즌 스탯을 받는다 — 홈/리더보드 화면과 동일한 병목이 이 화면(teamsWithAwards 계산용
    // teams)에도 있었음(services/multi/buildLeagueTeams.ts 주석 참고). 이 화면 자체의 "최근
    // 경기" 위젯(selectPlayerGameLog, 아래)은 경기 단위 원본이 그대로 필요해 그쪽은
    // includePbp 기본값(true)을 그대로 유지.
    const { data: statsByPlayer, isPending: statsLoading } = usePlayerSeasonStatsLeague(room?.id, allRosterIds);
    const selectLeagueTeams = useCallback(
        (raw: LeagueRawStatsData): Team[] => {
            const builtTeams = buildLeagueTeams(raw, leagueTeams, useCustomOverrides, statsByPlayer);
        ...
        [leagueTeams, useCustomOverrides, currentSimDate, room?.season_number, schedule, statsByPlayer],
    );

    const { data: teams = [], isPending: fetchLoading } = useLeagueRawStats(room?.id, allRosterIds, selectLeagueTeams, { includePbp: false });
    ...
    const isLoading = leagueLoading || gameLoading || fetchLoading || shortCodesLoading || statsLoading;
    ...
    // 불필요한 페이로드를 막기 위함, hooks/usePlayerCareerHistory.ts 주석 참고).
    // [2026-09-07] 로스터 선수(found)도 이 패턴으로 통일했다 — useLeagueRawStats가 더
    // 이상 career_history/tendencies를 안 가져오므로(팀 화면 등 대다수 호출부가 안 쓰는
    // 필드를 매번 250명+ 분량 같이 받던 걸 없앤 최적화), found/FA 구분 없이 "지금 보고 있는
    // 이 선수 한 명"만 아래에서 targeted로 조회해 덮어씌운다.
    //
    ...
    // playerId만 준비되면(shortCodesLoading만 확인) 무조건 켠다.
    ...
    // [2026-09-09] 올스타 선정 배지 — client 재계산이 불가능해(usePlayerAllStarAwards.ts
    // 주석 참고) 서버가 저장해둔 league_player_awards를 targeted 조회.
    const { data: playerAllStarAwards } = usePlayerAllStarAwards(room?.id, playerId, !shortCodesLoading);

    // 로스터에서 못 찾으면(어느 팀에도 없음) 드래프트풀 전체(poolPlayers)에서 FA로 한 번 더
    // 찾는다 — MultiFreeAgentView.tsx의 undraftedPlayers와 동일한 데이터 소스.
    const faPlayer = useMemo(
        () => !found ? poolPlayers.find(p => p.id === playerId) : undefined,
        [found, poolPlayers, playerId],
    );

    // found(로스터 선수)/faPlayer(FA) 구분 없이 위에서 targeted로 조회한
    // career_history/tendencies를 여기서 한 번에 덧씌운다.
    //
    // [버그 수정] useMemo 없이 매 렌더 새 객체를 만들면, targeted fetch가 늦게 도착해 이
    // 컴포넌트가 다시 렌더될 때마다 PlayerDetailView에 넘어가는 player prop의 참조가 계속
    // 바뀐다. 그런데 PlayerDetailView.tsx는 `useState(playerProp)` +
    // `useEffect(..., [playerProp.id, teamIdProp])`로 내부 상태를 미러링해서 id가 같으면
    // 재동기화를 안 하므로, "처음 진입 시엔 필드 없는 채로 굳어버리고 이후 새로고침해야만
    // 보이는" 문제가 생겼다. useMemo로 참조를 안정시키는 것만으로는 그 자체 버그를 못
    // 고치지만(참조가 바뀌어야 재동기화되므로), PlayerDetailView.tsx 쪽 effect 의존성도
    // 함께 고쳐서(playerProp 전체를 보게) 두 수정이 합쳐져야 완전히 해결된다.
    const basePlayer = found?.player ?? faPlayer;
    const playerWithTargetedFields = useMemo(() => {
        if (!basePlayer) return basePlayer;
        if (!playerCareerHistory?.length && !playerTendencies && !playerAllStarAwards?.length) return basePlayer;
        return {
            ...basePlayer,
            ...(playerCareerHistory?.length ? { career_history: playerCareerHistory } : {}),
            ...(playerTendencies ? { tendencies: playerTendencies } : {}),
            // 올스타 배지 — stampSeasonAwards()가 이미 채워둔 MVP/DPOY/올-NBA/올-디펜시브
            // 배열에 덧붙인다(교체 아님, HeaderAwardTrophies/PlayerAwardList가 type+season
            // 기준으로 중복 제거해 표시하므로 합쳐도 안전).
            ...(playerAllStarAwards?.length ? { awards: [...(basePlayer.awards ?? []), ...playerAllStarAwards] } : {}),
        };
    }, [basePlayer, playerCareerHistory, playerTendencies, playerAllStarAwards]);
