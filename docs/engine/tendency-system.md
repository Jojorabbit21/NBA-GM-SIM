...
| `services/game/engine/pbp/pbpTypes.ts` | LivePlayer.tendencies + zonePref 필드 |
| `services/game/engine/pbp/initializer.ts` | LivePlayer에 텐던시 할당, buildZonePref()로 zonePref 생성 |
...
| `services/game/engine/pbp/playTypes.ts` | ballDominance, playStyle 효과 (액터 선택), selectZone()에서 zonePref 70% 반영 |
