...
#### 4.8. 페이스(pace) 체력 트레이드오프 **[2026-07 신규]**
템포를 올리면 포제션이 짧아져 경기당 시도 횟수(득점 기회)가 늘어나는데, 그동안 이 이득에 대응하는 체력 대가가 전혀 없었음 — 5단계부터 선형으로 추가 체력 소모를 부여. 매 포제션 상시, 팀 전체 온코트 5인에게 균일 적용.
```
if pace >= 5:
    paceFatiguePenalty = 5 + (pace - 5) × 2   // %
...
5단계 미만은 페널티 없음(FG% 페널티의 임계값과 통일), 최대치(+15%)는 `fullCourtPress`의 최대 체력 페널티와 동일선상.
