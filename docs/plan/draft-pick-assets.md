...
### 8. 보호 조건 발동 로직 — `services/draft/draftOrderResolver.ts`

로터리 결과 확정 시점에 `resolveDraftOrder()`에서 자동 판정.
- `isProtectionTriggered()` + `revertPickOwnership()` + `addFallbackPick()`
- 1라운드/2라운드 동일 로직: `findTradedPicks()` → 슬롯 위치 확인 → 보호 판정
- `type='top'`, `threshold=N`: 슬롯 ≤ N → 보호 발동 (픽 원래 팀 잔류)
- `type='lottery'`: 슬롯 ≤ 14 → 보호 발동
- 보호 발동 시 fallback 이관, 결과: `ProtectionResult[]` + `updatedPickAssets`

### 9. 스왑 권리 행사 로직 — `services/draft/draftOrderResolver.ts`

로터리 결과 확정 시점에 `resolveDraftOrder()`에서 자동 실행.
- `findSwapPicks()` + `executeSwap()` + `findOwnPickIndex()`
- 보호 판정 후 실행 (보호 발동된 픽은 스왑 대상 제외)
- `beneficiary` 슬롯 > `origin` 슬롯 → 스왑 실행
- 결과: `SwapResult[]`

### 10. 드래프트 순번 배정 — `services/draft/lotteryEngine.ts` + `draftOrderResolver.ts`

로터리 엔진 → 드래프트 오더 해석기 → 60픽 최종 오더.
- `lotteryEngine.ts`: 30팀 `finalOrder` 생성 (로터리 + 플레이오프 역순)
- `draftOrderResolver.ts`: `resolveDraftOrder()` → 보호/스왑/소유권 반영 60픽 `ResolvedPick[]`
- 1라운드: `finalOrder` 순서, 2라운드: `finalOrder` 역순
- `offseasonEventHandler.ts`에서 로터리 직후 자동 호출
- `RookieDraftView`에서 `resolvedDraftOrder.picks`의 `currentTeamId[]` 사용
...
```
types/draftAssets.ts          — 타입 정의 (ResolvedPick, ProtectionResult, SwapResult, ResolvedDraftOrder 포함)
...
services/draft/
  lotteryEngine.ts            — 로터리 추첨 엔진 (30팀 finalOrder 생성)
  draftOrderResolver.ts       — 보호/스왑/소유권 반영 60픽 드래프트 오더 생성
services/persistence.ts       — 저장/로드 (league_pick_assets 파라미터)
hooks/useGameData.ts          — 상태 관리 (resolvedDraftOrder 포함)
views/FrontOfficeView.tsx     — UI (DraftPicksTab, PickTradeHistory)
views/DraftLotteryView.tsx    — 로터리 결과 + 권리행사/변동사항 표시
views/RookieDraftView.tsx     — 60픽 currentTeamId[] 기반 드래프트
