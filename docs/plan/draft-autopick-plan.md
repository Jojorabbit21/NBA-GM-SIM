...
| 1 | 특정 유저의 픽 타이머(`pickDurationSec`, 기본 30초)가 완전히 소진 | 그 유저를 오토픽 모드로 전환 — **이후 모든 픽**에 적용 (매번 다시 30초 기다리지 않음) | ⬜ 미구현(`onPickTimeout()`은 여전히 1픽만 대납) |
| 2 | 드래프트가 `waiting → active`로 전환되는 순간, `pickOrder`에 있는 유저가 방(WS)에 연결돼 있지 않음 | 즉시 오토픽 모드로 전환 | ✅ 구현 완료 (2026-07-27, `activate()`) |
| 3 | 오토픽 상태인 유저가 재접속(WS auth 성공) | 자동으로 오토픽 해제 → 수동 모드 복귀 | ⬜ 미구현(현재는 본인이 직접 토글 버튼을 눌러야 해제됨) |
