...
저작권 리스크 해소를 위해 **Claude AI의 NBA 지식 기반으로 632명 전원의 37개 속성을 독자 생성**하는 작업을 진행.

- 대상: 443명 로스터 선수 + 189명 FA 풀 = **632명**
- 속성: 37개 (offBallMovement, boxOut 추가)
- 스케일: 20-99
...
- 에이전트가 NBA 지식 기반으로 37개 속성을 JSON으로 출력
...
Supabase 적용용 SQL 생성.
- JSONB `||` 연산자로 37개 속성만 업데이트 (기존 필드 보존)
