-- ============================================================
-- respond_trade_offer() — league_events.sim_date 채우기 (v3)
--
-- 배경: 뉴스피드 헤더 날짜 범위 필터가 league_events.sim_date(인게임 날짜)를 기준으로
-- 조회하는데, 트레이드 이벤트는 game_id가 없어 simRunner.ts의 백필 대상이 아니고
-- v2까지도 이 컬럼을 안 채웠음 — 트레이드만 필터에서 계속 빠지는 문제. v2를 그대로
-- 복사하고 INSERT INTO league_events에 sim_date 컬럼 하나만 추가했다(그 외 로직 불변).
-- v_sim_date 스냅샷은 create_trade_offer()의 sim_date_at_creation과 동일 패턴
-- (rooms.sim_date, add_sim_date_to_trade_offers.sql 참고) — 다만 여기는 "제안 생성 시점"이
-- 아니라 "수락(accept) 시점"의 rooms.sim_date를 스냅샷한다(트레이드 성사 자체가 accept
-- 시점에 일어나므로).
