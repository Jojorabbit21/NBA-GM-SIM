-- ============================================================
-- respond_trade_offer() — sim_date 타입 캐스팅 버그 수정 (v4)
--
-- 배경: v3에서 league_events.sim_date(date 타입) 채우려고
-- `(SELECT sim_date FROM rooms WHERE id = v_o.room_id)`를 그대로 INSERT에 넣었는데,
-- rooms.sim_date는 text 컬럼이라 "column sim_date is of type date but expression is
-- of type text" 에러로 트레이드 accept 자체가 전부 실패함(실제 유저가 실전 트레이드
-- 수락 시도 중 발견). create_trade_offer()의 `v_sim_date date` 변수 대입은 plpgsql의
-- INTO 대입 시 암시적 I/O 캐스팅이 적용돼 문제없었지만, 이 INSERT 문의 타겟 컬럼
-- 리스트는 SQL 실행기가 처리해 text→date 캐스트가 등록돼 있지 않아 실패함.
-- v3를 그대로 복사하고 문제의 한 줄에만 `::date` 캐스트를 추가했다(그 외 로직 불변).
