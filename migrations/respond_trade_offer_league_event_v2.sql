-- ============================================================
-- respond_trade_offer() — league_events payload 구조화 (v2)
--
-- 배경: 멀티플레이어 뉴스피드 그리드 개편 — 트레이드 이벤트 payload가 headline 문자열
-- 뿐이라 새 카드 UI가 팀/선수 구조화 데이터를 못 씀. 이 파일은 respond_trade_offer_league_event.sql
-- (v1, 2026-08-29 적용)을 그대로 복사하고, accept 분기의 INSERT INTO league_events
-- jsonb_build_object 하나만 확장한 것 — 그 외 로직은 전혀 바뀌지 않았다. v1을 대체한다
-- (CREATE OR REPLACE라 같은 함수를 다시 덮어씀, 별도 함수가 생기지 않음).
--
-- payload에 팀 슬러그(teamA/teamB)와 이동한 선수 이름(aOut/bOut)을 구조화해서 담는다.
-- v_out/v_in은 player_id의 jsonb 배열뿐이라 이름이 없으므로 meta_players를 인라인 조회.
-- p.id::text 캐스팅 필수 — league_events.player_ids는 text[]인데 meta_players.id는 uuid.
