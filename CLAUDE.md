...
## 에이전트 모델 정책
서브에이전트(Task tool) 실행 시 작업 유형에 따라 모델을 지정할 것:
- **Explore/리서치 에이전트** → `haiku` (파일 탐색, 키워드 검색, 단순 조회)
- **Plan/설계 에이전트** → `opus` (아키텍처 설계, 복잡한 추론, 구현 계획)
- **코드 작성/Bash 에이전트** → `sonnet` (실제 코드 생성 및 수정)
