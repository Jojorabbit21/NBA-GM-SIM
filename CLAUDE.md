...
## 🚨 개발 히스토리 노트 (2026-07~)
**엔진/로직을 수정할 때는 `docs/history/dev-log.md`에 Before/After를 상세히 기록할 것.** 문제가 생겼을 때 이 문서만 보고 수동 복구할 수 있어야 함.
- 형식은 `docs/history/dev-log.md` 상단의 템플릿 참조 (배경 / 변경 파일 / Before / After / 검증 / 롤백 방법)
- client/server 미러 쌍 변경은 반드시 둘 다 기록 (하나만 롤백하면 미러가 깨짐)
- 최신 항목이 위로 오도록 역순 추가
- UI 전용 소소한 변경(색상/텍스트 크기 등)까지 전부 기록할 필요는 없음 — 엔진 로직/공식/상수처럼 되돌리기 까다로운 변경 위주
