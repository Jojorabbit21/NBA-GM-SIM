// startDraft.ts(실제 드래프트 풀 구성) / finalize.ts(OVR 정규화 muLeague 계산)가 공통으로
// 쓰는 meta_players 풀 필터. services/multi/draftPoolQuery.ts(client)와 미러 쌍 —
// 한쪽만 고치면 서버/클라 드래프트 풀 구성이 어긋나므로 항상 같이 수정할 것.
//
