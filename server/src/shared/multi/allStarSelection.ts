/**
 * allStarSelection.ts — 서버 미러.
 * 원본: utils/allStarSelection.ts. 로직/스코어링을 바꿀 땐 반드시 양쪽 다 같이 고칠 것
 * (client/server 미러 쌍 — dev-log.md 기록 대상). 원본과 100% 동일 로직, import 경로만 다름.
 *
 * 멀티플레이어 올스타 선발 로직 + 관련 키데이트.
 *
 * ## 스타터(선발 5명 — 가드 2 + 프론트코트 3) — 팬 투표, 누적 시뮬레이션
 * 올스타는 사실상 인기투표라는 전제로 설계:
 *   appeal = 인기도×0.6 + 시즌 성적(정규화)×0.25 + 브레이크아웃 보너스
 *   브레이크아웃 보너스 = max(0, 성적정규화 - 인기도) × 0.5
 * 득표수는 컨퍼런스당 400만 표(현실적인 시청자 스케일 근사치)를 각 포지션 그룹(가드/
 * 프론트코트) 내에서 appeal 기반 멱함수 가중치로 나눠 갖는 방식으로 계산 — 개별 유권자를
 * 실제로 반복 시뮬레이션하지 않고 "기대 득표수"를 직접 계산한다. voteProgress(0~1)로
 * 그 시점까지 누적된 득표수를 재현 가능하게 얻어 중간 발표(2~3회)에 재사용한다.
 *
 * ## 리저브(후보 7명) — 코치 투표, 성적 기반(인기도 미반영)
 * 스타터 제외 풀에서 awardVoting.ts의 scoreAllNBA로 100인 투표 시뮬레이션.
 */

import type { Team, Player } from '../types.ts';
import type { PlayerPopularity } from '../types/player.ts';
import {
...
export interface AllStarKeyDates {
     ...
     *  (allStarStart)과 분리(사용자 확인 — allStarStart+2일). 현재는 시뮬레이션 자체가
     *  미구현이라 값만 미리 계산해둠(향후 트리거용). */
    allStarThreePointContestDate: string;
    /** [2026-09-09] 덩크 컨테스트 실제 이벤트 실행일 — 3점 챌린지와 같은 "올스타 토요일
     *  나이트"에 함께 열리므로 allStarThreePointContestDate와 동일하게 allStarStart+2일.
     *  현재는 시뮬레이션 미구현, 서신 안내용. */
    allStarDunkContestDate: string;
    /** [2026-09-08] 라이징스타 챌린지 실제 경기 실행일 — allStarStart+1일(실제 NBA 순서:
     *  금 라이징스타 → 토 3점/덩크 → 일 본경기). 현재는 시뮬레이션 미구현, 서신 안내용. */
    allStarRisingStarsDate: string;
    /** [2026-09-08] 올스타 본경기 실제 실행일 — allStarStart+3일. 현재는 시뮬레이션 미구현,
     *  서신 안내용. */
