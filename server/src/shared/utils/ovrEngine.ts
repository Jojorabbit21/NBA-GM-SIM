...
/**
 * OVR Engine — 멀티플레이어 이식본 (Phase 2: 계산 로직 포팅 완료)
 * 원본: utils/ovrEngine.ts.
 *
 * LeagueDistribution/mapRawOVRToDisplayOVR/calculateLeagueDistribution/applyEliteRankCaps는
 * 원본에서도 실제로는 어디서도 호출되지 않는 죽은 코드로 확인되어(리그 분포 재보정은 라이브
 * 경로에서 적용되지 않음) 이식 대상에서 제외했다.
 */

import { getWeightConfigSync, getPositionConfigSync, getTagConfigSync, getLabelConfigSync } from '../services/admin/gameConfigService.ts';
...
// ─── 11) Future OVR (potential-based, separate from current OVR) ─────────────
...
// ─── 12) Archetype Display Labels (hardcoded fallback) ───────────────────────
// 관리자가 DB(archetypes.labels)에 커스텀 라벨을 저장하면 그게 우선이고, 없으면 이 값을 쓴다.
// 원본: utils/ovrEngine.ts (pages/PlayerEditorPage.tsx에 있던 걸 공용으로 뺀 것과 동일 소스).
