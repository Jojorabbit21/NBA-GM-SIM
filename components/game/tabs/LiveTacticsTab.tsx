            ...
            <div className={`grid grid-cols-5 gap-4${disabled ? ' pointer-events-none opacity-60' : ''}`}>
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <SliderControl label="골밑 공격" value={sliders.shot_rim} onChange={v => updateSlider('shot_rim', v)}
                        steps={SLIDER_STEPS.shot_rim} tooltip="가장 효율적인 슛 구역. 드라이브/컷 능력과 연계됩니다." />
                    <SliderControl label="중거리 슛" value={sliders.shot_mid} onChange={v => updateSlider('shot_mid', v)}
                        steps={SLIDER_STEPS.shot_mid} tooltip="중거리 슛은 효율이 낮습니다. 엘리트 슈터가 없다면 소극적으로 유지하세요." />
                </div>
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <SliderControl label="공격 스타일" value={sliders.playStyle} onChange={v => updateSlider('playStyle', v)}
                        steps={SLIDER_STEPS.playStyle} tooltip="히어로 볼은 스타의 1:1 창조력에 의존. 시스템 농구는 팀 패싱/무빙으로 오픈 슛을 만듭니다." />
                    <SliderControl label="공격 포인트" value={sliders.insideOut} onChange={v => updateSlider('insideOut', v)}
                        steps={SLIDER_STEPS.insideOut} tooltip="인사이드는 포스트업/컷/롤 강조. 아웃사이드는 캐치앤슛/팝/드라이브킥 강조." />
                    <SliderControl label="P&R 의존도" value={sliders.pnrFreq} onChange={v => updateSlider('pnrFreq', v)}
                        steps={SLIDER_STEPS.pnrFreq} tooltip="높을수록 픽앤롤 관련 플레이 비중이 커집니다. 핸들러+스크리너 콤비가 좋을수록 효과적." />
                </div>
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <SliderControl label="스위치 수비" value={sliders.switchFreq} onChange={v => updateSlider('switchFreq', v)}
                        steps={SLIDER_STEPS.switchFreq} tooltip="스위치할수록 미스매치 위험이 있지만 오픈 찬스는 줄어듭니다." />
                    ...
                    <SliderControl label="풀코트 프레스" value={sliders.fullCourtPress} onChange={v => updateSlider('fullCourtPress', v)}
                        steps={SLIDER_STEPS.fullCourtPress} tooltip="체력을 급격히 소모하며 턴오버를 유발합니다." />
                </div>
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <SliderControl label="헬프 수비" value={sliders.helpDef} onChange={v => updateSlider('helpDef', v)}
                        steps={SLIDER_STEPS.helpDef} tooltip="적극 지원할수록 페인트존 보호가 강해지지만, 외곽 3점슛을 허용합니다." />
