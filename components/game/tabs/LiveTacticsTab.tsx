            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-7 flex flex-col gap-4">
                    ...
                    <div className="bg-slate-900/40 border border-slate-800 rounded-2xl overflow-hidden">
                                    ...
                                    {shootingPlayers.map(player => {
                                        ...
                                        return (
                                            <button key={player.id} onClick={() => togglePlayer(player.id)}
                                                className={`w-full flex items-center justify-between px-2 py-1.5 rounded-lg transition-all border text-left ${isSelected ? 'bg-slate-800/80 border-slate-700' : 'hover:bg-slate-900 border-transparent hover:border-slate-800 text-slate-500'}`}>
                                                <div className="flex items-center gap-1.5">
                                                    <div className={`w-3 h-3 rounded flex items-center justify-center border transition-colors ${isSelected ? 'bg-indigo-600 border-indigo-500' : 'bg-transparent border-slate-600'}`}>
                                                        {isSelected && <Check size={8} className="text-white" />}
                                                    </div>
                                                    <span className={`text-[11px] font-bold truncate max-w-[80px] ${isSelected ? 'text-white' : 'text-slate-500'}`}>{player.name}</span>
                                                </div>
                                                <span className="text-[10px] font-mono text-slate-400">{player.made}/{player.total}</span>
                                            </button>
                                        );
                                    })}
                                </div>
                        ...
                        {/* Zone Stats */}
                        <div className="flex gap-2 px-4 py-2 border-t border-slate-800 justify-center">
                            <StatBox label="FG%" stat={zoneStats.fg} />
                            <div className="w-px h-6 bg-slate-800 self-center" />
                            <StatBox label="RA%" stat={zoneStats.ra} />
                            <div className="w-px h-6 bg-slate-800 self-center" />
                            <StatBox label="ITP%" stat={zoneStats.itp} />
                            <div className="w-px h-6 bg-slate-800 self-center" />
                            <StatBox label="MID%" stat={zoneStats.mid} />
                            <div className="w-px h-6 bg-slate-800 self-center" />
                            <StatBox label="3P%" stat={zoneStats.p3} />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-3">
                            <PlayTypePPPTable title="우리 팀 플레이타입 효율" data={userPPP} />
                        </div>
                        <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-3">
                            <PlayTypePPPTable title="상대 팀 플레이타입 분석" data={oppPPP} />
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-5 flex flex-col gap-1">
                    ...
                    <div className="grid grid-cols-2 gap-x-6">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <SliderControl label="헬프 수비 빈도" value={sliders.helpDef} onChange={v => updateSlider('helpDef', v)}
                                tooltip="높을수록 페인트존 보호가 강해지지만, 외곽 3점슛을 허용할 위험이 커집니다." fillColor="#6366f1" />
                            <SliderControl label="스위치 수비 빈도" value={sliders.switchFreq} onChange={v => updateSlider('switchFreq', v)}
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <SliderControl label="풀 코트 프레스" value={sliders.fullCourtPress} onChange={v => updateSlider('fullCourtPress', v)}
                                tooltip="체력을 급격히 소모하며 턴오버를 유발합니다. 가드 스태미나/스피드가 높을 때만 효과적." fillColor="#d946ef" />
                            <SliderControl label="지역 방어 빈도" value={sliders.zoneFreq}
