...
export const LiveTacticsTab: React.FC<LiveTacticsTabProps> = ({
    ...
    return (
        <div className="flex-1 overflow-hidden p-4 flex flex-col gap-4">
            ...
            {/* TOP: 슬라이더 5그룹 수평 배치                          */}
            {/* ══════════════════════════════════════════════════════ */}
            <div className={`grid grid-cols-4 gap-4${disabled ? ' pointer-events-none opacity-60' : ''}`}>

                {/* 1. 공격 철학 (게임 운영 + 코칭 철학 통합) */}
                <div className="flex flex-col gap-1">
                    <h4 className="text-[11px] font-black text-white uppercase tracking-widest mb-1">공격 철학</h4>
            ...
            <SliderGroupNotes notes={[
                ...
                { label: '공격 포인트', text: '인사이드는 포스트업/컷/롤 강조. 아웃사이드는 캐치앤슛/팝/드라이브킥 강조.' },
                { label: 'P&R 의존도', text: '높을수록 픽앤롤 관련 플레이 비중이 커집니다. 핸들러+스크리너 콤비가 좋을수록 효과적.' },
                { label: '3점 슛 빈도', text: '팀의 3점 시도 빈도를 결정합니다.' },
                { label: '골밑 공격', text: '가장 효율적인 슛 구역. 드라이브/컷 능력과 연계됩니다.' },
                { label: '중거리 슛', text: '중거리 슛은 효율이 낮습니다. 엘리트 슈터가 없다면 소극적으로 유지하세요.' },
                { label: '수비 압박', text: '타이트할수록 스틸 시도가 늘어나지만, 파울 트러블 위험이 커집니다.' },
                { label: '스위치 수비', text: '스위치할수록 미스매치 위험이 있지만 오픈 찬스는 줄어듭니다.' },
                { label: '픽앤롤 수비', text: '드랍: 림 보호(미드레인지 허용). 헷지: 핸들러 지연 후 복귀. 블리츠: 더블팀(턴오버 유발, 킥아웃 허용).' },
                { label: '풀코트 프레스', text: '체력을 급격히 소모하며 턴오버를 유발합니다.' },
                { label: '헬프 수비', text: '적극 지원할수록 페인트존 보호가 강해지지만, 외곽 3점슛을 허용합니다.' },
                { label: '지역 방어', text: '내선 수비력(블락+인사이드수비)이 강한 팀에게 유리합니다.' },
                { label: '수비 리바운드', text: '박스아웃할수록 세컨드찬스를 줄이지만, 속공 전환이 느려집니다.' },
            ]} />
