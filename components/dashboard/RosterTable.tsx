...
import { AlertCircle, GanttChartSquare, Timer } from 'lucide-react';
...
export const RosterTable: React.FC<RosterTableProps> = ({ 
  ...
  depthChart, onUpdateDepthChart
    ...
    const quarterBorder = "border-r-2 border-indigo-500/50"; // High visibility border
    ...
    return (
        <div className="flex flex-col h-full bg-slate-950/20 overflow-hidden">
            ...
            <div className="flex-shrink-0">
                <div className="px-6 py-2 bg-slate-900 border-y border-slate-800 flex items-center gap-2">
                ...
                <table className="w-full text-left border-separate border-spacing-0">
                    <thead className="sticky top-0 z-30 bg-slate-900 shadow-lg">
                        ...
                        <tr className="text-[9px] font-black text-slate-500 uppercase tracking-tighter">
                            ...
                            <th rowSpan={2} className="py-3 px-0 w-[50px] min-w-[50px] max-w-[50px] text-center border-b-2 border-indigo-500/50 border-r border-slate-800 sticky left-0 bg-slate-900 z-40">POS</th>
                            <th rowSpan={2} className="py-3 px-3 w-[160px] min-w-[160px] max-w-[160px] bg-slate-900 border-b-2 border-indigo-500/50 sticky left-[50px] z-40 border-r border-slate-800">PLAYER</th>
                            <th rowSpan={2} className="py-3 px-1 w-[40px] min-w-[40px] max-w-[40px] text-center border-b-2 border-indigo-500/50 border-slate-800 sticky left-[210px] bg-slate-900 z-40 border-r border-slate-800">OVR</th>
                            <th rowSpan={2} className={`py-3 px-1 w-[40px] min-w-[40px] max-w-[40px] text-center border-b-2 border-indigo-500/50 border-slate-800 sticky left-[250px] bg-slate-900 z-40 ${quarterBorder}`}>MIN</th>
                        ...
                        <tr className="text-[8px] font-black text-slate-600 uppercase tracking-tighter bg-slate-900 border-b-2 border-indigo-500/50">
                             {Array.from({length: 48}).map((_, i) => {
                                const isQuarterEnd = (i+1)%12 === 0 && (i+1) !== 48;
                                return (
                                    <th 
                                        key={i} 
                                        className={`w-8 min-w-[2rem] h-6 text-center border-b-2 border-indigo-500/50 ${isQuarterEnd ? quarterBorder : 'border-r border-slate-800/50'}`}
                                    >
                                        {i + 1}
                                    </th>
                                );
                                            ...
                                            return (
                                                <td 
                                                    ...
                                                    className={`
                                                        ...
                                                        ${isQuarterEnd ? quarterBorder : 'border-r border-slate-800/50'}
                                                        hover:bg-white/10
                                                    `}
                                                >
                                                    {active && (
