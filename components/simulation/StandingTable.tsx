...
interface StandingTableProps {
    ...
    teamStatusMap?: Record<string, 'clinched_playoff' | 'clinched_playin' | 'eliminated' | null>;
}

export const StandingTable: React.FC<StandingTableProps> = ({ 
    teamList, title, isConference = false, highlightColor = 'indigo', onTeamClick, mode, teamStatusMap 
}) => {
    ...
    return (
        <div className="flex flex-col gap-3">
            ...
            <h3 className={`text-lg font-black oswald uppercase tracking-wider text-${highlightColor}-400 px-1`}>
                {title}
            </h3>
            ...
            <div className="bg-slate-900/90 rounded-xl border border-slate-800 overflow-hidden shadow-xl">
                <Table className="rounded-none border-0 shadow-none !bg-transparent" fullHeight={false}>
                    <TableHead className="bg-slate-950 border-b border-slate-800">
                        <TableHeaderCell align="left" className="pl-6 pr-2 w-12 !rounded-none border-none bg-slate-950 py-3">#</TableHeaderCell>
                        <TableHeaderCell align="left" className="px-2 border-none bg-slate-950 !rounded-none py-3">TEAM</TableHeaderCell>
                        <TableHeaderCell align="center" className="px-2 w-10 border-none bg-slate-950 !rounded-none py-3">W</TableHeaderCell>
                        <TableHeaderCell align="center" className="px-2 w-10 border-none bg-slate-950 !rounded-none py-3">L</TableHeaderCell>
                        <TableHeaderCell align="center" className="px-2 w-16 border-none bg-slate-950 !rounded-none py-3">PCT</TableHeaderCell>
                        <TableHeaderCell align="center" className="pl-2 pr-6 w-14 !rounded-none border-none bg-slate-950 py-3">GB</TableHeaderCell>
                    </TableHead>
                    <TableBody>
                        {sorted.length > 0 ? sorted.map((t, i) => {
                            ...
                            if (teamStatusMap) {
                                const status = teamStatusMap[t.id];
                                if (status === 'clinched_playoff') {
                                    statusEmoji = '🔒';
                                    nameColorClass = "text-emerald-400";
                                } else if (status === 'clinched_playin') {
                                    statusEmoji = '🎟️';
                                } else if (status === 'eliminated') {
                                    statusEmoji = '❌';
                                    nameColorClass = "text-slate-600";
                                }
                            }
