...
export const RequirementCard: React.FC<RequirementCardProps> = ({ requirement, targetPlayers, onAccept, onPlayerClick }) => {
  ...
  const teamColor = TEAM_DATA[requirement.teamId]?.colors.primary || '#10b981';
       ...
       <div className="flex flex-col gap-6 relative z-10">
          ...
          <div className="space-y-3">
             <div className="flex justify-between items-center mb-1 px-1">
                <div className="text-[10px] font-black uppercase tracking-widest" style={{ color: teamColor !== '#000000' ? teamColor : '#34d399' }}>내 팀 요구 자산 ({requirement.players.length}인)</div>
                <div className="text-[10px] font-mono font-black text-slate-500">합계: {formatMoney(requirement.players.reduce((s,p)=>s+p.salary,0))}</div>
             </div>
