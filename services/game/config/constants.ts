...
export const SIM_CONFIG = {
    ...
    SHOOTING: {
        ...
        // [2026-08-01 Fix] 기존 커브는 dunk 80+에서 INSIDE_BASE_PCT(57%)와 합쳐지면 수비 반영
        // 전부터 이미 95% 하드캡 근처/초과(dunk 90=99.9%, 99=103.9%)라 수비가 사실상 무력화됨.
        // INSIDE_BASE_PCT는 Layup/Floater/Hook엔 문제없어(최대치도 캡까지 여유 충분) 그대로 두고,
        // 유독 폭이 큰 Dunk 커브만 약 60% 스케일로 압축 — dunk=99 기준 수비 반영 전 85%로 하향.
    ...
    SHOOTING_FOUL: {
        ...
        // [2026-09-14] 0.13 → 0.10, PAINT 0.10 → 0.08 — "수비 잘하는 빅맨이 오히려 파울 더 많이
        // 분다"는 사용자 관찰을 rim_share(방어시도 중 림존 비중) 실측으로 검증한 결과, intDef
        // 자체보다 "골밑을 얼마나 많이 지키느냐"가 파울률을 지배하고 있었음(BASE_RATE_RIM이
        // BASE_RATE_3PT의 5배가 넘어서, 스킬 할인보다 존 노출 자체가 훨씬 큰 변수) — 29팀 453명
        // 실제 로스터로 1280~10000경기 실측 후 확정(docs/history/dev-log.md 참고).
        BASE_RATE_RIM: 0.10,
        ...
        // [2026-09-10] Rim 0.6 → 0.5 — Paint와 동률로. 위 2026-07-30 튜닝(엠비드 FTA 과다 실측
        // 대응)의 연장선 — Paint보다 더 커야 할 근거가 약했던 Rim만 한 번 더 낮춰, 엘리트
        // 공격수의 drawFoul 보너스가 골밑에서만 유독 증폭되던 것을 완화(docs/history/dev-log.md 참고).
        ...
        INTERIOR_SKILL_CURVE: [   // Rim/Paint 전용 — intDef*0.65 + defConsist*0.35
            ...
            // [2026-09-14] 72 이상 구간만 비대칭으로 2배 확대(45/60은 원본 유지) — BASE_RATE 인하만
            // 으로는 "수비 좋은 빅맨이 덜 분다"는 방향성이 충분히 안 살아서(29팀 실측: 빅맨 내부
            // intDef 상위25%-하위25% pf/48 격차가 Control -0.06으로 거의 무의미), 상위 구간 할인만
            // 추가로 키움. 45~60을 그대로 둔 이유는 대칭 2배 버전이 약한 수비 빅맨의 페널티까지
            // 같이 키워 일부 선수 파울이 오히려 늘어나는 부작용이 있었기 때문(10000경기 실측으로
            // 비대칭 버전이 부작용 없이 30명 전원 개선되는 것까지 확인, docs/history/dev-log.md 참고).
