...
export function ensureFullStaff(data: LeagueCoachingData, teamIds: string[], tendencySeed: string): LeagueCoachingData {
    ...
    for (const teamId of teamIds) {
        ...
        if (!existing.headCoach) existing.headCoach = generateHeadCoach(teamId, tendencySeed);
        if (!existing.offenseCoordinator) existing.offenseCoordinator = generateOffenseCoordinator(`${tendencySeed}:oc:${teamId}`);
        if (!existing.defenseCoordinator) existing.defenseCoordinator = generateDefenseCoordinator(`${tendencySeed}:dc:${teamId}`);
        if (!existing.developmentCoach) existing.developmentCoach = generateDevelopmentCoach(`${tendencySeed}:dev:${teamId}`);
        if (!existing.trainingCoach) existing.trainingCoach = generateTrainingCoach(`${tendencySeed}:trainer:${teamId}`);
