...
export type MessageType = 'GAME_RECAP' | 'TRADE_ALERT' | 'TRADE_OFFER_RECEIVED' | 'TRADE_OFFER_RESPONSE' | 'INJURY_REPORT' | 'SUSPENSION' | 'LEAGUE_NEWS' | 'SEASON_REVIEW' | 'PLAYOFF_STAGE_REVIEW' | 'SEASON_AWARDS' | 'OWNER_LETTER' | 'HOF_QUALIFICATION' | 'FINALS_MVP' | 'REG_SEASON_CHAMPION' | 'PLAYOFF_CHAMPION' | 'SYSTEM' | 'SCOUT_REPORT' | 'OFFSEASON_REPORT' | 'PROSPECT_REVEAL' | 'LOTTERY_RESULT' | 'DRAFT_RESULT' | 'RETIREMENT_NEWS' | 'FA_SIGNING' | 'FA_RELEASE' | 'FA_LEAGUE_NEWS';
...
export const MESSAGE_FILTER_MAP: Record<MessageFilterCategory, MessageType[]> = {
    ...
    TRADE: ['TRADE_ALERT', 'TRADE_OFFER_RECEIVED', 'TRADE_OFFER_RESPONSE', 'FA_SIGNING', 'FA_RELEASE', 'FA_LEAGUE_NEWS'],
...
export interface DraftResultEntry {
    pickNumber: number;
    teamId: string;
    teamName: string;
    playerId: string;
...
export interface DraftResultContent {
    draftYear: number;
    myTeamId: string;
    myPickCount: number;
    entries: DraftResultEntry[];
}

export interface FASigningContent {
    playerId: string;
    playerName: string;
    position: string;
    ovr: number;
...
export interface FAReleaseContent {
    playerId: string;
    playerName: string;
    position: string;
    ovr: number;
